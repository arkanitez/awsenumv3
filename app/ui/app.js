...
const NODE_STYLES = [
  // Base node styles: NO background-image here to avoid invalid values being parsed
  { selector: 'node', style: {
      'label': 'data(label)',
      'font-size': 11,
      'text-wrap': 'wrap',
  ...
  // Only nodes with class `has-icon` get a background-image
...
function injectIcons(elements){
  // add icon + 'has-icon' class only when we have a valid icon
  return (elements || []).map(el => {
    ...
    if (icon) {
      el.data.icon = icon;
      // Append 'has-icon' to classes, keep existing ones
      const cls = (el.classes || '').trim();
      el.classes = (cls ? cls + ' ' : '') + 'has-icon';
    } else {
      // Ensure we don't leave an empty icon key around
...
async function handleEnumerateClick(){
  ...
  try {
    ...
    // sanitize edges, then apply icons + class
